...
GitHub et Stack Exchange autorisent aussi les lectures publiques CORS. Lorsque le quota IP partagé du Worker est indisponible, le frontend collecte donc les pages directement auprès de l’API officielle puis envoie au Worker une liste normalisée limitée à 10 000 contributions et 2 millions de caractères. Le Worker revalide les identifiants, applique les règles et marque `clientSourced=true` dans le reçu. Ce mode reste transparent : le serveur ne prétend pas avoir revérifié la liste fournie par le navigateur.
