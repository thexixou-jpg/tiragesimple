...
export function socialRulesSummary(provider: string, rules: SummaryRules): string[] {
  return [
    ...
    ...(provider === 'youtube' || provider === 'youtube_live' || provider === 'vimeo' || provider === 'soundcloud' || provider === 'mixcloud' || provider === 'twitch' || provider === 'kick' || provider === 'trovo' || provider === 'lemmy' || provider === 'reddit' || provider === 'github' || provider === 'gitlab' || provider === 'bitbucket' || provider === 'devto' || provider === 'hackernews' || provider === 'stackexchange' ? [
      ...
      rules.duplicateEntries ? `Chaque ${provider === 'stackexchange' ? 'contribution' : provider === 'youtube_live' || provider === 'kick' || provider === 'trovo' ? 'message' : 'commentaire'} éligible donne une chance ; un compte ne peut être sélectionné qu’une fois` : provider === 'twitch' ? 'Une seule chance par identifiant utilisateur Twitch' : provider === 'kick' ? 'Une seule chance par identifiant utilisateur Kick' : provider === 'trovo' ? 'Une seule chance par identifiant utilisateur Trovo' : provider === 'lemmy' ? 'Une seule chance par identité ActivityPub' : provider === 'reddit' ? 'Une seule chance par identifiant utilisateur Reddit' : provider === 'vimeo' ? 'Une seule chance par identité membre ou invité Vimeo' : provider === 'soundcloud' ? 'Une seule chance par URN utilisateur SoundCloud' : provider === 'mixcloud' ? 'Une seule chance par clé de profil Mixcloud' : provider === 'github' ? 'Une seule chance par identifiant utilisateur GitHub' : provider === 'gitlab' ? 'Une seule chance par identifiant utilisateur GitLab' : provider === 'bitbucket' ? 'Une seule chance par UUID de compte Bitbucket' : provider === 'devto' ? 'Une seule chance par identifiant membre DEV Community' : provider === 'hackernews' ? 'Une seule chance par identifiant utilisateur Hacker News sensible à la casse' : provider === 'stackexchange' ? 'Une seule chance par identifiant utilisateur Stack Overflow' : 'Une seule chance par identifiant de chaîne',
