...
export function socialRulesSummary(provider: string, rules: SummaryRules): string[] {
  return [
    ...
    ...(provider === 'youtube' || provider === 'youtube_live' || provider === 'twitch' || provider === 'kick' || provider === 'lemmy' || provider === 'github' || provider === 'stackexchange' ? [
      ...
      rules.duplicateEntries ? `Chaque ${provider === 'stackexchange' ? 'contribution' : provider === 'youtube_live' || provider === 'kick' ? 'message' : 'commentaire'} éligible donne une chance ; un compte ne peut être sélectionné qu’une fois` : provider === 'twitch' ? 'Une seule chance par identifiant utilisateur Twitch' : provider === 'kick' ? 'Une seule chance par identifiant utilisateur Kick' : provider === 'lemmy' ? 'Une seule chance par identité ActivityPub' : provider === 'github' ? 'Une seule chance par identifiant utilisateur GitHub' : provider === 'stackexchange' ? 'Une seule chance par identifiant utilisateur Stack Overflow' : 'Une seule chance par identifiant de chaîne',
      ...
      provider === 'twitch' ? 'Messages envoyés, follow, abonnement et rôles non utilisés' : provider === 'kick' ? 'Follow, abonnement et présence silencieuse non vérifiés' : provider === 'lemmy' ? 'Votes et abonnement à la communauté non vérifiés' : provider === 'github' ? 'Réactions, commits et statut de contributeur non vérifiés' : provider === 'stackexchange' ? 'Votes, réputation et statut de réponse acceptée non utilisés' : provider === 'youtube_live' ? 'Abonnement, likes, dons et messages supprimés non vérifiés' : 'Abonnement à la chaîne et likes non vérifiés',
    ] : provider === 'mastodon' ? [
