...
export function socialRulesSummary(provider: string, rules: SummaryRules): string[] {
  return [
    ...
    ...(provider === 'youtube' || provider === 'youtube_live' || provider === 'twitch' || provider === 'kick' || provider === 'trovo' || provider === 'lemmy' || provider === 'github' || provider === 'stackexchange' ? [
