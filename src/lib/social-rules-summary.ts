...
export function socialRulesSummary(provider: string, rules: SummaryRules): string[] {
  return [
    ...
    ...(provider === 'youtube' || provider === 'youtube_live' || provider === 'vimeo' || provider === 'soundcloud' || provider === 'mixcloud' || provider === 'twitch' || provider === 'kick' || provider === 'trovo' || provider === 'lemmy' || provider === 'reddit' || provider === 'github' || provider === 'gitlab' || provider === 'devto' || provider === 'hackernews' || provider === 'stackexchange' ? [
