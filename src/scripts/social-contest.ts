...
for (const root of document.querySelectorAll<HTMLElement>('[data-social-contest]')) {
  const provider = root.dataset.provider === 'bluesky' ? 'bluesky' : root.dataset.provider === 'mastodon' ? 'mastodon' : root.dataset.provider === 'lemmy' ? 'lemmy' : root.dataset.provider === 'reddit' ? 'reddit' : root.dataset.provider === 'github' ? 'github' : root.dataset.provider === 'gitlab' ? 'gitlab' : root.dataset.provider === 'bitbucket' ? 'bitbucket' : root.dataset.provider === 'devto' ? 'devto' : root.dataset.provider === 'hackernews' ? 'hackernews' : root.dataset.provider === 'stackexchange' ? 'stackexchange' : root.dataset.provider === 'youtube_live' ? 'youtube_live' : root.dataset.provider === 'vimeo' ? 'vimeo' : root.dataset.provider === 'soundcloud' ? 'soundcloud' : root.dataset.provider === 'mixcloud' ? 'mixcloud' : root.dataset.provider === 'twitch' ? 'twitch' : root.dataset.provider === 'kick' ? 'kick' : root.dataset.provider === 'trovo' ? 'trovo' : root.dataset.provider === 'discord' ? 'discord' : 'youtube';
  ...
  const readRules = () => {
    ...
    const commentProvider = provider === 'youtube' || provider === 'youtube_live' || provider === 'vimeo' || provider === 'soundcloud' || provider === 'mixcloud' && mixcloudInteraction === 'comments' || provider === 'kick' || provider === 'trovo' || provider === 'lemmy' || provider === 'reddit' || provider === 'github' || provider === 'gitlab' || provider === 'bitbucket' || provider === 'devto' || provider === 'hackernews' || provider === 'stackexchange';
    const replyProvider = provider === 'youtube' || provider === 'vimeo' || provider === 'lemmy' || provider === 'reddit' || provider === 'bitbucket' || provider === 'devto' || provider === 'hackernews';
