...
for (const root of document.querySelectorAll<HTMLElement>('[data-social-contest]')) {
  const provider = root.dataset.provider === 'bluesky' ? 'bluesky' : root.dataset.provider === 'mastodon' ? 'mastodon' : root.dataset.provider === 'lemmy' ? 'lemmy' : root.dataset.provider === 'reddit' ? 'reddit' : root.dataset.provider === 'github' ? 'github' : root.dataset.provider === 'gitlab' ? 'gitlab' : root.dataset.provider === 'stackexchange' ? 'stackexchange' : root.dataset.provider === 'youtube_live' ? 'youtube_live' : root.dataset.provider === 'vimeo' ? 'vimeo' : root.dataset.provider === 'soundcloud' ? 'soundcloud' : root.dataset.provider === 'mixcloud' ? 'mixcloud' : root.dataset.provider === 'twitch' ? 'twitch' : root.dataset.provider === 'kick' ? 'kick' : root.dataset.provider === 'trovo' ? 'trovo' : root.dataset.provider === 'discord' ? 'discord' : 'youtube';
  ...
  const analyze = async () => {
    ...
    if (!eligibleUrl(url)) { say(`Utilisez ${provider === 'twitch' ? 'un login ou un lien de chaîne Twitch' : provider === 'kick' ? 'le login ou le lien de votre chaîne Kick connectée' : provider === 'trovo' ? 'un login ou un lien de chaîne Trovo' : provider === 'discord' ? 'le lien complet d’un message Discord de serveur' : `un lien ${provider === 'bluesky' ? 'bsky.app vers une publication' : provider === 'mastodon' ? 'provenant d’une instance Mastodon prise en charge' : provider === 'lemmy' ? 'vers un post d’une instance Lemmy prise en charge' : provider === 'reddit' ? 'vers une publication Reddit publique' : provider === 'vimeo' ? 'vers une vidéo Vimeo publique' : provider === 'soundcloud' ? 'vers une piste SoundCloud publique' : provider === 'mixcloud' ? 'vers une émission Mixcloud publique' : provider === 'github' ? 'vers une issue ou pull request GitHub publique' : provider === 'gitlab' ? 'vers une issue ou merge request GitLab.com publique' : provider === 'stackexchange' ? 'vers une question Stack Overflow publique' : 'YouTube vers une vidéo ou un Short'}`}.`); return; }
    resetDraw();
  ...
  const readRules = () => {
    ...
    const commentProvider = provider === 'youtube' || provider === 'youtube_live' || provider === 'vimeo' || provider === 'soundcloud' || provider === 'mixcloud' && mixcloudInteraction === 'comments' || provider === 'kick' || provider === 'trovo' || provider === 'lemmy' || provider === 'reddit' || provider === 'github' || provider === 'gitlab' || provider === 'stackexchange';
