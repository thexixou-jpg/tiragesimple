import { parseDiscourseUrl } from '../lib/discourse-forums';
...
for (const root of document.querySelectorAll<HTMLElement>('[data-social-contest]')) {
  ...
  const eligibleUrl = (value: string) => {
    try {
      ...
      if (provider === 'discourse') return Boolean(parseDiscourseUrl(value));
  ...
  const analyze = async () => {
    ...
    try {
      ...
      root.querySelector<HTMLElement>('[data-publication-meta]')!.textContent = [provider === 'discourse' ? parseDiscourseUrl(url)?.name : provider === 'stackexchange' ? parseStackExchangeUrl(url)?.name : '', data.authorName, date].filter(Boolean).join(' · ');
  ...
  const readRules = () => {
    ...
    const commentProvider = provider === 'youtube' || provider === 'youtube_live' || provider === 'vimeo' || provider === 'soundcloud' || provider === 'mixcloud' && mixcloudInteraction === 'comments' || provider === 'peertube' || provider === 'kick' || provider === 'trovo' || provider === 'lemmy' || provider === 'reddit' || provider === 'github' || provider === 'gitlab' || provider === 'bitbucket' || provider === 'discourse' || provider === 'devto' || provider === 'hackernews' || provider === 'stackexchange' || provider === 'wordpress';
    const replyProvider = provider === 'youtube' || provider === 'vimeo' || provider === 'peertube' || provider === 'lemmy' || provider === 'reddit' || provider === 'bitbucket' || provider === 'discourse' || provider === 'devto' || provider === 'hackernews' || provider === 'wordpress';
  ...
  const poll = async (id: string, current: number) => {
    try {
      ...
      progress.textContent = `${state.progress_current} ${provider === 'twitch' ? 'comptes présents analysés' : provider === 'youtube_live' || provider === 'kick' || provider === 'trovo' ? 'messages du chat analysés' : provider === 'discord' ? 'réactions Discord analysées' : provider === 'youtube' || provider === 'vimeo' || provider === 'peertube' || provider === 'lemmy' || provider === 'reddit' || provider === 'bitbucket' || provider === 'discourse' || provider === 'devto' || provider === 'hackernews' || provider === 'wordpress' ? 'commentaires et réponses analysés' : provider === 'soundcloud' || provider === 'github' || provider === 'gitlab' ? 'commentaires analysés' : provider === 'mixcloud' ? 'interactions Mixcloud analysées' : provider === 'stackexchange' ? 'contributions analysées' : 'interactions analysées'} · ${state.participant_count} comptes éligibles`;
