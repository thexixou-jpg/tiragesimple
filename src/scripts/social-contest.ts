...
for (const root of document.querySelectorAll<HTMLElement>('[data-social-contest]')) {
  ...
  const readRules = () => {
    ...
    const replyProvider = provider === 'youtube' || provider === 'vimeo' || provider === 'lemmy' || provider === 'reddit' || provider === 'bitbucket' || provider === 'devto' || provider === 'hackernews' || provider === 'wordpress';
