...
export function normalizeRules(input: Partial<ContestRules>): ContestRules {
  ...
  return {
    ...
    interaction: input.interaction === 'reposts' ? 'reposts' : input.interaction === 'answers' ? 'answers' : input.interaction === 'comments' ? 'comments' : input.interaction === 'listeners' ? 'listeners' : 'likes',
    clientSourced: false,
