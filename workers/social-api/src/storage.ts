...
export async function reserveProviderRequest(env: Env, provider: string): Promise<void> {
  ...
  const limit = provider === 'youtube' || provider === 'youtube_live' ? 6000 : provider === 'twitch' || provider === 'kick' || provider === 'trovo' ? 8000 : provider === 'github' ? (env.GITHUB_API_TOKEN ? 4000 : 50) : provider === 'stackexchange' ? (env.STACKEXCHANGE_API_KEY ? 9000 : 250) : 10000;
