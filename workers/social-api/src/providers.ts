...
export const providerCapabilities: Record<ProviderId, ProviderCapabilities> = {
  ...
  discourse: { comments:true,likes:false,reposts:false,mentions:false,followers:false,replies:true },
...
export function providerStatus(env: Env): Record<ProviderId, string> {
  return {
    ...
    discourse: env.DISCOURSE_ENABLED === 'true' && Boolean(env.DB && env.SOCIAL_IMPORT_QUEUE) ? 'enabled' : 'disabled',
