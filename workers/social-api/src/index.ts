...
<a href="https://tiragesimple.fr/">TirageSimple</a><p><span class="tag">Résultat partagé · ${escapeHtml(draw.platform === 'youtube' ? 'YouTube' : draw.platform === 'mastodon' ? 'Mastodon' : 'Bluesky')}</span></p>
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const providerMatch = pathname.match(/^\/v1\/(youtube|bluesky|mastodon)\/(publication|imports)$/u);
    if (request.method === 'POST' && providerMatch) {
      try {
        ...
        const publication = provider === 'youtube' ? await getYouTubePublication(input.url, env)
          : provider === 'bluesky' ? await getBlueskyPublication(input.url, env) : await getMastodonPublication(input.url, env);
