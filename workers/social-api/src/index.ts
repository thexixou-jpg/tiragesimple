...
<a href="https://tiragesimple.fr/">TirageSimple</a><p><span class="tag">Résultat partagé · ${escapeHtml(draw.platform === 'youtube' ? 'YouTube' : draw.platform === 'youtube_live' ? 'YouTube Live' : draw.platform === 'vimeo' ? 'Vimeo' : draw.platform === 'soundcloud' ? 'SoundCloud' : draw.platform === 'mixcloud' ? 'Mixcloud' : draw.platform === 'twitch' ? 'Twitch' : draw.platform === 'kick' ? 'Kick' : draw.platform === 'trovo' ? 'Trovo' : draw.platform === 'discord' ? 'Discord' : draw.platform === 'mastodon' ? 'Mastodon' : draw.platform === 'lemmy' ? 'Lemmy' : draw.platform === 'reddit' ? 'Reddit' : draw.platform === 'github' ? 'GitHub' : draw.platform === 'gitlab' ? 'GitLab' : draw.platform === 'bitbucket' ? 'Bitbucket' : draw.platform === 'devto' ? 'DEV Community' : draw.platform === 'hackernews' ? 'Hacker News' : draw.platform === 'stackexchange' ? 'Stack Overflow' : 'Bluesky')}</span></p>
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const providerMatch = pathname.match(/^\/v1\/(youtube|youtube_live|vimeo|soundcloud|mixcloud|discord|bluesky|mastodon|lemmy|reddit|github|gitlab|bitbucket|devto|hackernews|stackexchange)\/(publication|imports)$/u);
